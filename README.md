...
- **Announcements styling**: Regular (coach-authored) announcements render as white cards with a subtle primary-blue left border, dark readable text, a small muted “Entire Team” or group badge under the content, and a relative timestamp such as `3 hours ago`. Auto-generated system announcements render as slightly tinted cards without the blue border, with a tiny gray `System` label above the message, smaller italic muted text, and the same relative timestamp treatment so they are visually distinct from coach-authored updates.
- **Team forms**: Team-scoped flows like **Create workout** and **Create group** reuse the same fixed sidebar and top bar layout as the main team dashboard, so coaches always stay anchored in team context while adding new workouts or position groups.
